...
public class DRLrulesBuilder 
{
    ...
    private static File TBoxFile  = new File("D-KB/DKBTBox.ttl");
        ...
        //DRLRulesBuilder also generates equivalent rules for the SHACL reasoner described in [Robaldo et al., 2023].
        //Specifically, the method buildSHACLRulesForSHACLreasonerFromRobaldoetal2023 re-processes the DKBRules and TBoxFile
        //after they are created and written to disk. It then produces two corresponding files in the SHACLreasonerDirectory:
        //TBoxDLRandDKB.ttl and DLRandDKBrules.ttl. These files contain the same semantic information, but are formatted for 
        //compatibility with the SHACL reasoner from [Robaldo et al., 2023].
    private static File SHACLreasonerDirectory = new File("../SHACLreasonerFromRobaldoetal2023");
...
//***************************************************************************************************************************************
    ...
    )
    {
        for(Rule sparqlRule:sparqlRules)
        {
            ...
            if(sparqlRule.ID.indexOf("statements36Formula1")!=-1)
                sparqlRule=sparqlRule;
            ...
            for(String[] triple:triples)
            {
                ...
                if((triple[1].compareTo("rdf:type")==0)&&(triple[0].trim().indexOf("<<")==0)&&(triple[2].indexOf("rdf:type :false")!=-1))
                    eventualitiesOnModality.put(triple[0].substring(triple[0].indexOf("?"),triple[0].length()).trim(), "");
            }
            ...
            for(String[] triple:triples)
                if((eventualitiesOnModality.get(triple[0])!=null)&&(triple[1].compareTo("rdf:type")==0)&&
                   ((triple[2].compareTo(":Rexist")!=0)&&(triple[2].compareTo(":A-Obligatory")!=0)&&(triple[2].compareTo(":M-Obligatory")!=0)&&
                    (triple[2].compareTo(":A-Prohibited")!=0)&&(triple[2].compareTo(":M-Prohibited")!=0)&&(triple[2].compareTo(":Obligatory")!=0)&&(triple[2].compareTo(":Permitted")!=0)))
                    EventualityTypes.put(triple[2],"");//quiiii
	...
	"WHERE{?ep rdf:type :ProcessPersonalData. ?ep :has-time ?t1. ?t1 time:hasEnd ?t1e. "+
        ...
        "NOT EXISTS{?t2r rdf:type time:Interval; time:hasBeginning ?t1e; time:hasEnd ?t2er; :is-without-delay ?t1.}}\"^^xsd:string].\n");
    ...
    {
        if(SHACLreasonerDirectory.exists()==false)return;
        ...
        try(FileOutputStream out=new FileOutputStream(new File(SHACLreasonerDirectory.getAbsolutePath()+"/TBoxDLRandDKB.ttl"))){RDFDataMgr.write(out,RDFDataMgr.loadModel(new File("DLRrules/DLR.ttl").getAbsolutePath()).add(TBoxDLRandDKB),RDFFormat.TURTLE);}

        printRules(rules,new File(SHACLreasonerDirectory.getAbsolutePath()+"/DLRandDKBrules.ttl"));
    }
    ...
    private static String[] searchForTarget(String rule)throws Exception
    {
        ...
        else if(rule.indexOf("?ed rdf:type :Deliver.")!=-1)return new String[]{"sh:targetClass",":Deliver","?e"};
        else if(rule.indexOf("?erdl rdf:type :Deliver.")!=-1)return new String[]{"sh:targetClass",":Deliver","?e"};
        ...
        else if(rule.indexOf("?ec :checks-exception-in-11-2 (?y ?t1 ?w).")!=-1)return new String[]{"sh:targetSubjectsOf",":checks-exception-in-11-2","?ec"};
        else if(rule.indexOf("?ex :checks-exception-in-11-2-identifyW (?y ?t1 ?w ?eid)")!=-1)return new String[]{"sh:targetSubjectsOf",":checks-exception-in-11-2-identifyW","?ex"};
        ...
        else if(rule.indexOf("?rs :is-reason-for ?en.")!=-1)return new String[]{"sh:targetSubjectsOf",":is-reason-for","?rs"};
        else if(rule.indexOf("?erc :has-object-state :not-electronic.")!=-1)return new String[]{"sh:targetSubjectsOf",":has-object-state","?erc"};
        
        else 
            throw new Exception("No predicate can be identified for sh:targetClass or sh:targetSubjectsOf");
    }
