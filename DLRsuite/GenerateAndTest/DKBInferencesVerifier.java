        ...
        for(String rexistFact:rexistsOnRootOfCluster)
        {
            ...
            try(QueryExecution qe=QueryExecutionFactory.create(QueryFactory.create(query),dataset))
            {
                ...
                else if((regulatives.isEmpty()==true)&&(results.isEmpty()==false))
                {
                    ...
                    {
                        if(DKBidRegulative.compareToIgnoreCase(result.getLiteral("dsDKBid").getString())!=0)continue;
                ...
                else if((regulatives.isEmpty()==false)&&(results.isEmpty()==false))
                {
                    String DfulfilledRules = "";
                    for(QuerySolution result:results)
                    {
                        if(DKBidRegulative.compareToIgnoreCase(result.getLiteral("dsDKBid").getString())==0){DfulfilledRules="";break;}
                        DfulfilledRules+=result.getLiteral("dsDKBid").getString()+", ";
                    }
                    if(DfulfilledRules.isEmpty()==false)
                        throw new Exception("Rule violation for "+DKBidRegulative+" on "+rexistFact+": the fact satisfies the rule(s) with ID(s) "+DfulfilledRules+"but does not satisfy the rule with ID "+DKBidRegulative);
                }
            }
        }
    }
